...
{
  m_postFrame            = (m_postFrame + 1) % FRAME_IN_FLIGHT;
...
bool VulkanSample::needToDenoise()
{
  if(m_denoiseApply)
  {
    if(m_pcRay.frame == m_maxFrames)
...
void VulkanSample::copyImagesToCuda(const VkCommandBuffer& cmdBuf)
{
  if(needToDenoise())
  {
#ifdef NVP_SUPPORTS_OPTIX7
...
void VulkanSample::copyCudaImagesToVulkan(const VkCommandBuffer& cmdBuf)
{
  if(needToDenoise())
  {
#ifdef NVP_SUPPORTS_OPTIX7
...
void VulkanSample::setImageToDisplay()
{
  bool showDenoised =
...
void VulkanSample::createSwapchain(const VkSurfaceKHR& surface,
                                   uint32_t            width,
                                   uint32_t            height,
...
//
//
void VulkanSample::submitWithTLSemaphore(const VkCommandBuffer& cmdBuf)
{
...
//
void VulkanSample::submitFrame(const VkCommandBuffer& cmdBuf)
{
...
void VulkanSample::createGbuffers()
{
  m_alloc.destroy(m_gAlbedo);
  m_alloc.destroy(m_gNormal);
  m_alloc.destroy(m_gDenoised);
