...
#elif USE_SLANG
#include "_autogen/shader_computeMain.spirv.h"
const auto& comp_shd = std::vector<uint32_t>{std::begin(shader_computeMain), std::end(shader_computeMain)};
#else
...
class EmptyElement : public nvvkhl::IAppElement
{
public:
  ...
  void createShaderObjectAndLayout()
  {
    ...
    shaderCreateInfos.push_back(VkShaderCreateInfoEXT{
        ...
        .pCode                  = comp_shd.data(),
        .pName                  = USE_HLSL ? "computeMain" : "main",
        .setLayoutCount         = 1,
...
int main(int argc, char** argv)
{
  ...
  app->addElement(std::make_shared<EmptyElement>());                 // Add our sample to the application
