...
private:
  ...
  void createRtxPipeline()
  {
    ...
    bindings.addBinding(B_triMat, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, (uint32_t)m_bMeshes.size(), VK_SHADER_STAGE_ALL);
    ...
    // Pipeline layout
    nvvk::createPipelineLayout(m_device, &m_pipelineLayout, {m_descriptorPack.getLayout()}, {push_constant});
  ...
  void writeRtDesc()
  {
    ...
    writes.append(m_descriptorPack.makeWrite(B_vertex), vertex_desc.data());
    writes.append(m_descriptorPack.makeWrite(B_index), index_desc.data());
    writes.append(m_descriptorPack.makeWrite(B_triMat), trimat_desc.data());
