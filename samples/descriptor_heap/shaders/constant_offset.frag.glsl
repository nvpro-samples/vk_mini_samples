...
// The shader stays in set/binding form. Mapping in createShaders() maps this
// unsized array to the resource heap with HEAP_WITH_CONSTANT_OFFSET. The
// shader computes texIdx = baseFaceTexIdx + faceIdx and indexes heapTextures[]
// through the mapped set/binding declaration.
layout(set = 0, binding = 0) uniform texture2D heapTextures[];
layout(set = 0, binding = 1) uniform sampler heapSamplers[];
