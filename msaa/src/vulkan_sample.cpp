...
void VulkanSample::createScene(const std::string& filename)
{
  ...
  {  // Graphic
    ...
    createMsaaRender(m_msaaSamples);
...
//
void VulkanSample::updateUniformBuffer(const VkCommandBuffer& cmdBuf)
{
...
void VulkanSample::createGraphicsPipeline()
{
  ...
  nvvk::GraphicsPipelineGeneratorCombined gpb(m_device, m_pipelineLayout, pass);
...
//
// #MSAA
void VulkanSample::createMsaaRender(VkSampleCountFlagBits samples)
{
  m_alloc.destroy(m_msaaColor);
  m_alloc.destroy(m_msaaDepth);
  vkDestroyImageView(m_device, m_msaaColorIView, nullptr);
...
void VulkanSample::renderUI()
{
  ...
  if(ImGui::Checkbox("Use MSAA", &useMsaa))
  {
    ...
    // If the MSAA sample is different from VK_SAMPLE_COUNT_4_BIT
    //m_alloc.destroy(m_msaaColor);
    //m_alloc.destroy(m_msaaDepth);
    //vkDestroyImageView(m_device, m_msaaColorIView, nullptr);
    //vkDestroyImageView(m_device, m_msaaDepthIView, nullptr);
