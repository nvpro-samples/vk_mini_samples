...
//-----------------------------------------------------------------------
MaterialEval evaluateMaterial(in GltfShadeMaterial material, in vec3 normal, in vec3 tangent, in vec3 bitangent, in vec2 uv, inout uint seed)
{
  ...
  res.transparent = (rand(seed) > res.albedo.a);
  return res;
}
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
vec3 lambertian(vec3 f0, vec3 f90, vec3 diffuseColor, float VdotH, float metallic)
{
...
vec3 bsdfSample(in MaterialEval state, in vec3 V, inout uint seed)
{
  vec3  N           = state.normal;
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
vec3 pbrSample(in MaterialEval state, vec3 V, inout vec3 L, inout float pdf, inout uint seed)
{
  // Find boucing ray
  L = bsdfSample(state, V, seed);
